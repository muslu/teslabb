...
def TersRenkCevir( x ):
	"""HSV renk alanı zıt rengini döndürür"""
	x = uzun_hex( x )
	if x == '000000':
		return 'ffffff'
	elif x == 'ffffff':
...
def lightness( x, value ):
	logging.info( str( value ) + "  ll " + str( x ) )
	x = uzun_hex( x )
	h, s, v = hex_to_hsv( x, False ) if len( x ) == 6 else x
...
def saturation( x, value ):
	logging.info( str( value ) + "  ss " + str( x ) )
	x = uzun_hex( x )
	h, s, v = hex_to_hsv( x, False ) if len( x ) == 6 else x
...
def color( name ):
	try:
		return IsimlendirilenRenkler[name]
	except:
		return ''
...
register.filter( 'TersRenkCevir', TersRenkCevir )
register.filter( 'uzun_hex', uzun_hex )
register.filter( 'kisa_hex', kisa_hex )
